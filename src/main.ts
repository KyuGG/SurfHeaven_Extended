...
;(async function () {
	...
	function fetch_map_rank(map_name) {
		...
		make_request('https://api.surfheaven.eu/api/maprecord/' + map_name + '/' + _id, data => {
			...
			rank_elem.title =
				...
				' at ' +
				date_completed.toLocaleTimeString()
			...
			if (data) {
				...
				make_request('https://api.surfheaven.eu/api/maps', data2 => {
					...
					if (settings.completions_bar_chart) {
						...
						completionsbytier_title.innerHTML =
							"<span style='float:left; margin-left:20px;'>Completions by Tier</span><div style='display: inline-block; width: 10px; height: 10px; background-color:CornflowerBlue;'></div> Map <div style='display: inline-block; width: 10px; height: 10px; background-color: DarkSeaGreen;'></div> Bonus</span>"
						...
						new Chartist.Bar(
							...
							{
								labels: labels,
								series: [completions_data, bonus_completions_data],
							},
							{
								stackBars: false,
								axisY: {
						...
						completions_tbody.innerHTML =
							'<tr><th>Tier</th><th>Maps</th><th>Map %</th><th>Bonuses</th><th>Bonus %</th></tr>'
						for (let j = 0; j < 7; j++) {
							let _tier = j + 1
							let map_percent = Math.floor((completions[j] / total[j]) * 100)
							let bonus_percent = Math.floor(
								(bonus_completions[j] / bonus_total[j]) * 100
							)
							completions_tbody.innerHTML +=
								'<tr><td>T' +
								_tier +
	...
	function get_id() {
		...
		if (use_custom) {
			...
			if (unsafeWindow.localStorage.getItem('cached_id') != null) {
				...
				console.log('no saved id found in local storage, checking dom')
			}
			...
			try {
				...
				make_request('https://api.surfheaven.eu/api/id', data => {
					...
					if (data) {
						...
						if (
							!showed_id_prompt &&
							(!unsafeWindow.localStorage.getItem('waiting_for_id') ||
								unsafeWindow.localStorage.getItem('waiting_for_id') == 'false')
						) {
							let last_ditch_id = prompt(
								"The script couldn't figure out your ID. Please go to your profile, you will be prompted to confirm there. Alternatively you can type your username here, and then select your profile",
								'Your Steam username'
							)
							...
							if (
								...
								unsafeWindow.location.href =
									'https://surfheaven.eu/search/' + last_ditch_id
							}
						}
					}
				})
	...
	function fetch_country_rank(id) {
		make_request('https://api.surfheaven.eu/api/playerinfo/' + id, data => {
			if (data) {
				...
				country_rank_html.innerHTML =
					' <i style="font-size:1em" class="pe pe-7s-star c-accent fa-3x"></i> Country Rank: ' +
					country_rank +
					'/' +
					country_rank_total
	...
	function fetch_time_spent(id) {
		make_request('https://api.surfheaven.eu/api/playerinfo/' + id, data => {
			if (data) {
				...
				ts_td.innerHTML =
					'<strong class="c-white">' + time_spent_spec + '</strong> Hours in spec'
				ts_td2.innerHTML =
					'<strong class="c-white">' + time_spent_loc + '</strong> Hours in loc'
			...
			if (data) {
				data.forEach(map => {
					if (map.track == 0) {
						...
						if (map.completions != undefined) {
							// LOOKING AT YOU surf_fornax b7
		...
		make_request('https://api.surfheaven.eu/api/records/' + id + '/', records => {
			make_request('https://api.surfheaven.eu/api/servers', servers => {
				if (Array.isArray(servers)) {
					...
					const rank_cells = Array(rowsCount)
						.fill()
						.map((_, i) => {
							if (table.rows[i + 1]) {
					...
					const bonus_cells = Array(rowsCount)
						.fill()
						.map((_, i) => {
							if (table.rows[i + 1]) {
					...
					records.forEach(record => {
						const record_found =
							servers.findIndex(server => server.map === record.map) >= 0
						if (record_found) {
					...
					servers.forEach((server, i) => {
						try {
							...
							if (server.mapinfo) {
								server_row[2].innerHTML = `<a href="https://surfheaven.eu/map/${
									server.map
								}">${server.map}</a> <small>(T${server.mapinfo.tier}) ${
									server.mapinfo.type == 0 ? 'Linear' : 'Staged'
								} </small>`
								...
								if (rec) {
									...
									if (map_record) {
										...
										const top_percent =
											unsafeWindow.localStorage.getItem('rank_threshold') /
											100
										const top_x = Math.ceil(
											server.mapinfo.completions * top_percent
										)
										let element_with_color = document.createElement('span')
										element_with_color.textContent = map_record.rank
										let complete_rank = document.createTextNode(
											' / ' + server.mapinfo.completions
										)
										...
										const txt = document.createTextNode(
											'0 / ' + server.mapinfo.completions
										)
										rank_cells[i].appendChild(txt)
									}
									const bonus_completes = rec.reduce(
										(value, record) =>
											record && record.track > 0 ? value + 1 : value,
										0
									)
									const txt2 = document.createTextNode(
										bonus_completes + ' / ' + server.mapinfo.bonus
									)
			...
			servers.forEach((server, server_index) => {
				try {
					...
					server_completions_2.forEach(completion => {
						...
						h5_elem.style =
							'margin-top: 10px;margin-bottom: 10px;font-size: 14px;font-family: inherit;display: block;   margin-inline-start: 0px;margin-inline-end: 0px;line-height: 1.1; text-align: end;'
						...
						if (!records || !records[completion.track]) {
							...
							const top_percent =
								unsafeWindow.localStorage.getItem('rank_threshold') / 100

							if (
								records[completion.track].rank / completion.completions <=
								top_percent
							) {
								rank_text = `<span class="text-success">${
									records[completion.track].rank
								}  </span>`
							} else {
								rank_text = `<span class="text-danger">${
									records[completion.track].rank
								}  </span>`
							}
						...
						rank_elem.style =
							'margin-top: 10px;margin-bottom: 10px;font-size: 14px;font-family: inherit;display: block;   margin-inline-start: 0px;margin-inline-end: 0px;line-height: 1.1;'
	...
	function dashboard_page() {
		...
		make_request('https://api.surfheaven.eu/api/playerinfo/' + get_id() + '/', c => {
			...
			make_request('https://api.surfheaven.eu/api/ctop/' + country + '/100', data => {
				...
				var target_div = document.querySelector(
					'.content > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1)'
				)
				var top_players_div = target_div.querySelector(
					'.content > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1)'
				)
				var top_wr_holders_div = target_div.querySelector(
					'.content > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(1) > div:nth-child(2)'
				)
				...
				ctop_panel_heading_div.style =
					'padding-bottom:0px; padding-top:6px; padding-left:6px;'
	...
	function insert_user_rated_maps_table() {
		...
		GM_xmlhttpRequest({
			...
			onload: function (response) {
				...
				thead.innerHTML =
					'<tr><th>Map name</th><th>Ratings</th><th>Difficulty</th><th>Fun</th><th>Unit</th><th>Tech</th></tr>'
				...
				for (let map in maps) {
					...
					difficulty_rating.innerHTML = Number(maps[map].difficulty_rating).toFixed(
						maps[map].difficulty_rating % 1 === 0 ? 0 : 2
					)
					fun_factor_rating.innerHTML = Number(maps[map].fun_factor_rating).toFixed(
						maps[map].fun_factor_rating % 1 === 0 ? 0 : 2
					)
					unit_rating.innerHTML = Number(maps[map].unit_rating).toFixed(
						maps[map].unit_rating % 1 === 0 ? 0 : 2
					)
					tech_rating.innerHTML = Number(maps[map].tech_rating).toFixed(
						maps[map].tech_rating % 1 === 0 ? 0 : 2
					)
	...
	function servers_page() {
		...
		switchButton.innerHTML =
			'<label class="switch"><input class="my-checkbox" type="checkbox"><span class="slider"></span><span class="labels" data-on="MANUAL" data-off="AUTO"></span></label>'
		...
		queue_button.onclick = function () {
			...
			queue_button.innerHTML =
				'<i class="fa fa-spinner fa-pulse fa-lg fa-fw"></i> Queueing...  (click to cancel)'
				...
				for (let i = 0; i < data.length; i++) {
					if (data[i].players.length == 0 && !data[i].ip.includes('mayhem')) {
						if (region_code == 'ALL' || data[i].region == region_code) {
							...
							found = true
							console.log(
								'Found empty server: ' + data[i].name + ' (' + data[i].ip + ')'
							)
	...
	function profile_page() {
		var steam_profile_url =
			document.querySelector('.m-t-xs > a:nth-child(2)') == null
				? document.querySelector('.m-t-xs > a:nth-child(1)').href
				: document.querySelector('.m-t-xs > a:nth-child(2)').href
		...
		make_request(
			'https://api.surfheaven.eu/api/playerinfo/' + current_profile_id,
			function (data) {
				if (data[0].vip != true) {
					const gift_vip_button = document.createElement('button')
					gift_vip_button.className = 'btn btn-success btn-xs'
					get_id() == current_profile_id
						? (gift_vip_button.innerHTML = "<i class='fab fa-paypal'></i> Buy vip")
						: (gift_vip_button.innerHTML = "<i class='fa fa-gift'></i> Gift VIP")
					gift_vip_button.onclick = function () {
						gift_vip(
							steam_profile_url.split('/')[steam_profile_url.split('/').length - 1]
						)
					}
		...
		if (get_id() == current_profile_id) {
			if (settings.user_effects) {
				...
				user_effect_button.innerHTML =
					'<i class="fas fa-magic"></i> <span class="candycane-rainbow">User effect<span>'
				user_effect_button.onclick = function () {
					show_overlay_window(
						'Set user effect',
						create_color_grid(original_username, btoa(current_profile_id))
					)
				}
		...
		let compare_target_div = document.querySelector(
			'div.col-sm-12:nth-child(3) > div:nth-child(1) > div:nth-child(1)'
		)
		...
		let bonus_ranks = []
		make_request(
			'https://api.surfheaven.eu/api/records/' + current_profile_id,
			function (data) {
				for (let i = 0; i < data.length; i++) {
					if (data[i].track != 0) bonus_ranks.push(data[i].rank)
				}
				let avg_brank = Math.ceil(
					bonus_ranks.reduce((a, b) => a + b, 0) / bonus_ranks.length
				)
				var stats_table = document.querySelector('.medium > tbody:nth-child(1)')
				var stats_table_rows = stats_table.children
				var points_td = document.createElement('td')
				points_td.innerHTML =
					'<strong class="c-white">' + avg_brank + '</strong> Avg Bonus Rank'
		...
		if (current_profile_id != get_id()) {
			...
			let common_uncompleted_maps_target_div = document.querySelector(
				'div.col-sm-12:nth-child(4) > div:nth-child(1) > div:nth-child(1)'
			)
			common_uncompleted_maps_target_div.insertBefore(
				common_uncompleted_maps_button,
				common_uncompleted_maps_target_div.children[1]
			)
		}
	}
	...
	function create_color_grid(username, c) {
		...
		function add_color(color) {
			...
			test_string.innerHTML = `<span class="${
				'candycane-custom-' + selected_colors.join('-')
			}">${username}</span>`
			selected_colors_text.innerHTML = selected_colors.join(', ')
		}
		return color_grid
			...
			for (let i = 0; i < player_data[0].length; i++) {
				for (let j = 0; j < player_data[1].length; j++) {
					if (player_data[0][i].map == player_data[1][j].map) {
						...
						map_data.push([
							player_data[0][i].map,
							player_data[0][i].tier,
							player_data[0][i].rank,
							player_data[0][i].points,
							player_data[1][j].rank,
							player_data[1][j].points,
						])
	...
	function insert_dropdown_stats(map, friends = false) {
		...
		function insert_glyphs() {
			// local variation of the same function, i'll fix this later
			records_table.forEach(record => {
				...
				toggle_glyph.style =
					'float: right; margin-right: 10px;margin-left: 20px; cursor: pointer;'
				toggle_glyph.href = '#'
		...
		make_request('https://api.surfheaven.eu/api/records/' + map + '/' + id, function (data) {
			...
			if (data) {
				...
				if (session_mapseconds == 0) {
                    ...
                    &emsp;&emsp;Time in map: <strong>${format_time_noms(
						session_mapseconds
					)}</strong> in session (<strong>${format_time_noms(
						total_mapseconds
					)}</strong> total) <br>
                    &emsp;&emsp;Time in level: <strong>${format_time_noms(
						session_leveltime
					)}</strong> in session (<strong>${format_time_noms(
						total_leveltime
					)}</strong> total) <br>
				...
				hidden_row.innerHTML =
					"<p style='margin-bottom:0px;'>&emsp;&emsp;Cant get stats for map " +
					map +
					'</p>'
			}
		})
	...
	function format_time_noms(time: number) {
	...
	function insert_rating(map) {
		...
		function toggle_rating_modal() {
			if (!document.querySelector('#rating_modal')) {
				...
				difficulty_label_small.innerHTML =
					'<br>How hard do you find this map relative to its tier?'
				...
				difficulty_rating_input.addEventListener('input', function () {
					difficulty_label.innerHTML =
						'Difficulty ' +
						difficulty_rating_input.value +
						'/5' +
						'<br><small>How hard do you find this map relative to its tier?</small>'
				})
				...
				fun_rating_input.addEventListener('input', function () {
					fun_label.innerHTML =
						'Fun ' +
						fun_rating_input.value +
						'/5' +
						'<br><small>How fun do you find this map?</small>'
				})
				...
				unit_rating_input.addEventListener('input', function () {
					unit_rating_label.innerHTML =
						'Unit ' +
						unit_rating_input.value +
						'/5' +
						'<small><br>Is the map unit heavy?</small>'
				})
				...
				tech_rating_input.addEventListener('input', function () {
					tech_rating_label.innerHTML =
						'Tech ' +
						tech_rating_input.value +
						'/5' +
						'<small><br>How technical is the map?</small>'
				})
			...
			GM_xmlhttpRequest({
				...
				onload: function (response) {
					...
					if (json.error_message) {
						...
						panel_heading_span.innerHTML +=
							' (0 Ratings, be the first to rate this map!)'
					} else {
						map_ratings = json
						...
						difficulty_panel_body_text.innerHTML =
							'Difficulty ' + difficulty.toFixed(difficulty % 1 === 0 ? 0 : 2) + '/5'
						fun_panel_body_text.innerHTML =
							'Fun ' + fun.toFixed(fun % 1 === 0 ? 0 : 2) + '/5'
						unit_panel_body_text.innerHTML =
							'Unit ' + unit.toFixed(unit % 1 === 0 ? 0 : 2) + '/5'
						tech_panel_body_text.innerHTML =
							'Tech ' + tech.toFixed(tech % 1 === 0 ? 0 : 2) + '/5'
				...
				if (!ratings.error_message) {
					...
					for (let i = 0; i < ratings.length; i++) {
						...
						if (i >= max_ratings_to_show) {
							ratings_div_list_item.innerHTML = `... and ${
								ratings.length - max_ratings_to_show
							} others`
							ratings_div.appendChild(ratings_div_list_item)
							break
						}
						make_request(
							'https://api.surfheaven.eu/api/playerinfo/' + ratings[i],
							data => {
								if (data) {
	...
	function insert_friend_rankings(map) {
		...
		make_request(`https://api.surfheaven.eu/api/records/${map}/0`, data => {
			...
			children.length > 1
				? (friends_ranking_button.innerHTML = 'Friends')
				: (friends_ranking_button.innerHTML = 'Show friends')
			...
			friends_ranking_button.onclick = function (e) {
				...
				if (children.length > 1) {
					...
					button_text == 'Show friends'
						? (friends_ranking_button.innerHTML = 'Show global')
						: (friends_ranking_button.innerHTML = 'Show friends')
				}
			}
			...
			for (let i = 0; i < friend_ranks.length; i++) {
				...
				// date
				td = document.createElement('td')
				td.innerHTML = new Date(friend_ranks[i][2])
					.toISOString()
					.substr(0, 19)
					.replace('T', ' ')
				tr.appendChild(td)
	...
	function insert_map_page_tag_list(current_map_name) {
		...
		function append_to_last_row(tag) {
			...
			curr_row = Math.floor(
				target_div.getElementsByClassName('tag').length / max_tags_per_row
			)
	...
	function open_map_tag_menu() {
		...
		for (let i = 0; i < all_tags.length; i++) {
			...
			tag_link.onclick = () => {
				...
				let tag_regex_pattern = tag_array
					.map(tag => {
						return `(?=.*${tag}\\b)`
					})
					.join('')
	...
	function open_tag_selection_window(map_name, from_map_page = false) {
		if (document.getElementById('tag_selection_modal') != null) {
			...
			modal
				...
				.getElementsByClassName('btn-primary')[0].onclick = () => {
				...
				if (from_map_page) {
					...
					if (map_tags_div == undefined) {
						...
						document
							.getElementsByClassName('container-fluid')[2]
							.appendChild(map_tags_div)
					}
		...
		make_request('https://api.surfheaven.eu/api/rankinfo/', function (data) {
			for (let i = 0; i < data.length; i++) {
				if (data[i].rank < own_rank) {
                    ...
                    Points needed for [<span style="color: ${
						GROUP_COLORS[GROUP_THRESHOLDS.indexOf(next_rank)]
					}">${[
						GROUP_NAMES[GROUP_THRESHOLDS.indexOf(next_rank)],
					]}</span>] : ${points_until_next_rank}`
					let insert_after = document.querySelector('.media > h5:nth-child(4)')
					insert_after.parentNode.insertBefore(
						points_until_next_rank_element,
	...
	function insert_points_per_rank(map_name) {
		...
		if (settings.map_cover_image) {
			total_completions_element = document.querySelector(
				'table.table:nth-child(2) > tbody:nth-child(2) > tr:nth-child(2) > td:nth-child(2) > strong:nth-child(2)'
			)
		} else {
			total_completions_element = document.querySelector(
				'table.table-responsive > tbody:nth-child(1) > tr:nth-child(1) > td:nth-child(1) > strong:nth-child(1)'
			)
		}
		...
		for (let i = 0; i < ranks.length; i++) {
			make_request(
				'https://api.surfheaven.eu/api/maprank/' + map_name + '/' + ranks[i] + '/0',
				function (data) {
					try {
		...
		setTimeout(function () {
			...
			for (let i = 0; i < Math.ceil(ranks.length / 2); i++) {
				...
				if (
					points[i + Math.ceil(points.length / 2)] !== undefined &&
					points[i + Math.ceil(points.length / 2)] !== 0
				) {
					let cell2 = row.insertCell(1)
					cell2.innerHTML = `#<strong class="c-white">${
						ranks[i + Math.ceil(points.length / 2)]
					}: ${points[i + Math.ceil(points.length / 2)]}</strong> pts`
				}
			}
			let table_body = document.createElement('tbody')
			let target_div = settings.map_cover_image
				? document.querySelector('div.col-md-3:nth-child(4)')
				: document.querySelector('div.col-md-3:nth-child(2)')
			let upper_table = settings.map_cover_image
				? document.querySelector('table.table:nth-child(2)')
				: document.querySelector('table.table-responsive')
	...
	function insert_map_picture(map_name) {
		...
		let map_link =
			'https://github.com/Sayt123/SurfMapPics/raw/Maps-and-bonuses/csgo/' + map_name + '.jpg'
		...
		col_1.style =
			'background-color: rgba(0, 0, 0, 0.4); margin-right: 4.15%; border-radius: 1rem; height: 300px; box-shadow: 0px 2px 0px 0px #f6a821;'
		col_2.style =
			'background-color: rgba(0, 0, 0, 0.4); margin-right: 4.15%; border-radius: 1rem; height: 300px; box-shadow: 0px 2px 0px 0px #f6a821;'
		if (settings.cp_chart)
			col_3.style =
				'background-color: rgba(0, 0, 0, 0.4); border-radius: 1rem; height: 300px; box-shadow: 0px 2px 0px 0px #f6a821;'
	}
	...
	function cp_chart() {
		...
		var top_panel_row = document.querySelector(
			'.panel-c-warning > div:nth-child(1) > div:nth-child(1)'
		)
		var map_info_col = document.querySelector(
			'.panel-c-warning > div:nth-child(1) > div:nth-child(1) > div:nth-child(1)'
		)
		var map_stats_col = document.querySelector(
			'.panel-c-warning > div:nth-child(1) > div:nth-child(1) > div:nth-child(2)'
		)
		...
		make_request(
			'https://api.surfheaven.eu/api/checkpoints/' + current_map_name,
			function (data) {
				var cp_labels = ['Start']
				var cp_series = [0]
				var own_series = []
				var all_series = [cp_series, own_series]
				var cp_chart = new Chartist.Line(
					'.ct-chart',
					{
						labels: cp_labels,
						series: [],
				...
				if (data[0].steamid != get_id()) {
					make_request(
						'https://api.surfheaven.eu/api/checkpoints/' +
							current_map_name +
							'/' +
							get_id(),
						function (data2) {
							if (data2.length > 0) {
								...
								for (var i = 0; i < data2.length; i++) {
									...
									if (i != 0 && i < data2.length - 1 && !isNaN(diff))
										cp_labels[i] = (diff > 0 ? '+' : '') + diff
								}
								...
								var end_diff = (
									own_series[own_series.length - 1] -
									cp_series[cp_series.length - 1]
								).toFixed(2)
		...
		GM_xmlhttpRequest({
			...
			onload: function (response) {
				if (response.status == 200) {
					...
					var image_with_style = image_full.replace(
						'/>',
						"style='border-radius: 5px;margin-right:10px;float:left;' />"
					)
					var media_div = document.querySelector('.media')
	...
	function open_settings_menu() {
		...
		backup_everything_button.onclick = () => {
			download(
				...
				'surfheaven_extended_settings.json',
				'text/plain'
			)
		}
		...
		localstorage_size_p.textContent =
			'Localstorage size: ' + localstorage_size_text() + '  / ~5MB'
	...
	function show_all_map_raters() {
		...
		make_request('https://iloveur.mom/surfheaven/get_all_ids.php', function (response) {
			...
			response.forEach(rater => {
				make_request(
					'https://api.surfheaven.eu/api/playerinfo/' + rater,
					function (player) {
